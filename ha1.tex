...
\begin{enumerate}
    ...
    \item[(b)] 
    Wenn $M$ in $z_3$ kommt, werden danach alle 'a's mit 'b's ersetzt (von rechts nach links) bis alle Buchstaben durchgegangen werden, wodurch $M$ in Zustand $z_4$ gelangt. Dann wird der Buchstabe rechts vom Lesekopf entweder ein 'b' oder ein 'c' sein.

    Aus $\delta$ folgt offensichtlich: $M$ hält falls $w\in \{a^nb^mc^k\mid n,m,k \in \NN\}$. Jedes Wort was sich nicht an die Reihenfolge hält, terminiert ohne den Endzustand zu erreichen.
    \item[(c)] 
